...
function $readTableInfo(): TableInfo | null {
  ...
  const cellNode = $findMatchingParent(selection.anchor.getNode(), $isTableCellNode);
...
export default function TableActionsPlugin() {
  ...
  const btnStyle: React.CSSProperties = {
    padding: '4px 7px', border: 'none', borderRadius: 4,
    cursor: 'pointer', fontSize: 11, fontWeight: 500,
    background: 'transparent', color: 'var(--nim-text)',
    fontFamily: 'inherit', lineHeight: 1,
  };

  const countStyle: React.CSSProperties = {
    padding: '2px 6px', fontSize: 12, fontWeight: 600,
    color: 'var(--nim-text)', minWidth: 22, textAlign: 'center',
  };
  ...
  return (
    ...
    >
      <svg width="13" height="13" viewBox="0 0 16 16" style={{ fill: 'none', stroke: 'var(--nim-text-muted)', strokeWidth: 1.5, verticalAlign: 'middle', marginRight: 4, flexShrink: 0 }}>
      ...
      {/* Row controls */}
      <button onClick={handleRemoveRow} disabled={rows <= 1} style={{...btnStyle, opacity: rows <= 1 ? 0.35 : 1}} title="Delete row">−R</button>
      <span style={countStyle}>{rows}</span>
      <button onClick={handleAddRow} style={btnStyle} title="Add row">+R</button>

      <div style={{ width: 1, height: 14, background: 'var(--nim-border)', margin: '0 3px' }} />

      {/* Column controls */}
      <button onClick={handleRemoveCol} disabled={cols <= 1} style={{...btnStyle, opacity: cols <= 1 ? 0.35 : 1}} title="Delete column">−C</button>
      <span style={countStyle}>{cols}</span>
      <button onClick={handleAddCol} style={btnStyle} title="Add column">+C</button>

      <div style={{ width: 1, height: 14, background: 'var(--nim-border)', margin: '0 3px' }} />

      {/* Alignment */}
      <button onClick={() => setAlignment('left')} style={btnStyle} title="Align left">⇤</button>
      <button onClick={() => setAlignment('center')} style={btnStyle} title="Align center">⇔</button>
      <button onClick={() => setAlignment('right')} style={btnStyle} title="Align right">⇥</button>
    </div>
  );
}
